...
let career;
try {
  career = JSON.parse(read(careerPath));
  ...
  pass('public career model JSON syntax and required fields');
} catch (error) {
...
if (failures.length === 0) pass('required human and machine outputs exist');

const resumeJsonPath = outputPath('/resume.json');
if (exists(resumeJsonPath)) {
  try {
    ...
    pass('resume.json syntax and release contract');
  } catch (error) {
...
if (exists(dist)) {
  ...
  pass('internal links and canonical URL generation');
  ...
  pass('JSON-LD emitted and parseable on intended pages');
}
...
if (exists(llmsPath)) {
  ...
  pass('llms.txt exists with canonical discovery references');
}
...
if (exists(sitemapPath)) {
  ...
  pass('sitemap exists and contains required canonical human pages');
}
...
if (exists(robotsPath)) {
  ...
  pass('robots.txt exists and references sitemap');
}
...
if (exists(dist)) {
  ...
  pass('generated output excludes known private-source markers');
}
...
try {
  ...
  pass('tracked-file obvious secret signature scan');
} catch (error) {
